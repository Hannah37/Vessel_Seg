...
def build_model(args, device):
    ...
    try:
        model = SwinUNETR(img_size=roi_size, in_channels=1, out_channels=2, feature_size=args.feature_size, use_checkpoint=args.use_checkpoint)
    except TypeError:
        model = SwinUNETR(in_channels=1, out_channels=2, feature_size=args.feature_size, use_checkpoint=args.use_checkpoint)
...
def main():
    ...
    def run_full_validation(epoch, tag="Full validation"):
        ...
        with torch.no_grad():
            for val_data in val_full_loader:
                ...
                with torch.cuda.amp.autocast(enabled=args.amp):
                    val_outputs = sliding_window_inference(val_inputs, roi_size=roi_size, sw_batch_size=args.sw_batch_size, predictor=model, overlap=0.25)
                val_outputs = [post_pred(i) for i in decollate_batch(val_outputs)]
                val_labels = [post_label(i) for i in decollate_batch(val_labels)]
                dice_metric(y_pred=val_outputs, y=val_labels)
            metric = dice_metric.aggregate().item()
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        print(f"Epoch {epoch}/{args.max_epochs} finished | train_loss={epoch_loss:.6f} | lr={current_lr:.6g} | epoch_time={format_seconds(time.time()-epoch_start)} | elapsed={format_seconds(elapsed_total)} | ETA={format_seconds(eta)} | estimated_finish={finish.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
