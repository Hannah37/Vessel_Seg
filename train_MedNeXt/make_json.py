...
BASE_DIR = Path("/vf/users/drdcad/Hyuna/projects/vessel_seg/code/train_MedNeXt")
...
def subject_id_from_case_id(case_id):
...
for fold in range(N_FOLDS):
    ...
    # leakage check
    train_subject_set = {x["subject_id"] for x in train_items}
    val_subject_set = {x["subject_id"] for x in val_items}

    overlap = train_subject_set & val_subject_set
    if overlap:
    ...
    print(
        ...
        f"train_subjects={len(train_subject_set)}, val_subjects={len(val_subject_set)}"
    )
...
# =========================
out = {
    "description": "SBO vessel segmentation 5-fold split for MedNeXt",
    "modality": "CT",
    "labels": {
        "0": "background",
        "1": "vessel",
