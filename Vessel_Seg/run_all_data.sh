...
# 이미 OUT_DIR에 생성된 결과 CASE_ID 저장
for OUT_FILE in "$OUT_DIR"/*_gt.nii.gz; do
    out_filename=$(basename "$OUT_FILE")
    DONE_CASE_ID="${out_filename%_gt.nii.gz}"
    DONE_CASES[$DONE_CASE_ID]=1
done

echo "Already processed output cases:"
echo "${#DONE_CASES[@]}"
...
for SEG_DIR in "${SEG_DIRS[@]}"; do
    ...
    for SEG in "$SEG_DIR"/*.nii.gz; do
        ...
        # pred_sma_smv 중에서 이미 OUT_DIR에 결과가 있는 case는 제외
        if [ "$SEG_DIR" = "$SEG_DIR_PRED" ] && [[ -n "${DONE_CASES[$CASE_ID]+x}" ]]; then
            echo "[SKIP] Already processed pred case: $CASE_ID"
            continue
        fi

        # gt_sma_smv_nnunet을 우선 사용.
        # 같은 CASE_ID가 이미 앞 폴더에서 발견되었으면 뒤 폴더 것은 skip.
        if [[ -n "${SEEN_CASES[$CASE_ID]+x}" ]]; then
            echo "[SKIP] Duplicate CASE_ID already seen: $CASE_ID"
            echo "$CASE_ID first=${SEEN_CASES[$CASE_ID]} duplicate=$SEG" >> "$DUPLICATE_LIST"
            continue
        fi
