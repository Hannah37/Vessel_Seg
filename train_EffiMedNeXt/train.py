...
# Official EfficientMedNeXt repo import.
# Make sure EFFICIENT_MEDNEXT_ROOT is in PYTHONPATH, e.g.
# export PYTHONPATH=/path/to/EfficientMedNeXt:${PYTHONPATH:-}
...
def case_id_from_item(item):
    """Extract nnU-Net case id from split JSON entry.
    ...
    # nnU-Net raw image channel suffix: CASE_0000.nii.gz -> CASE
...
class NnUNetPreprocessedDataset(TorchDataset):
    ...
    mode='patch': return foreground-biased patches per case.
    """
...
def flatten_patch_batch(batch_data, device):
    ...
    return inputs.to(device, non_blocking=True), labels.to(device, non_blocking=True).float()
...
def sigmoid_dice_loss(logits, target, eps=1e-6):
...
def build_efficient_mednext_model(args):
    """Build a single EfficientMedNeXt model.

    The official repository exposes model scales T/S/M/L via
    create_efficient_mednext(in_channels, out_channels, scale, ...).
    We use out_channels=1 for binary vessel segmentation and sigmoid loss.
    """
...
def get_args():
    ...
    parser.add_argument("--full_val_every", type=int, default=100)
    ...
    parser.add_argument("--num_patch_samples", type=int, default=1)
    ...
    # EfficientMedNeXt settings.
    parser.add_argument("--model_size", type=str, default="S", choices=["T", "S", "M", "L"])
    parser.add_argument("--feature_size", type=int, default=32)
    parser.add_argument("--n_decoder_channels", type=int, default=32)
    parser.add_argument("--kernel_sizes", type=int, nargs=3, default=[1, 3, 5])
...
def main():
    ...
    val_patch_files = val_files[: args.quick_val_max_cases] if args.quick_val_max_cases > 0 else val_files
    ...
    print(f"ROI: {roi_size}", flush=True)
    print(f"EfficientMedNeXt size: {args.model_size}", flush=True)
    print(f"EfficientMedNeXt feature_size: {args.feature_size}", flush=True)
    print(f"EfficientMedNeXt n_decoder_channels: {args.n_decoder_channels}", flush=True)
    print(f"EfficientMedNeXt kernel_sizes: {args.kernel_sizes}", flush=True)
    print(f"Deep supervision: {args.deep_supervision}", flush=True)
    ...
    model = build_efficient_mednext_model(args).to(device)
    ...
    def compute_loss(logits, labels):
        ...
        loss_dice = sigmoid_dice_loss(logits, labels)
        loss_bce = bce_loss_fn(logits.float(), labels.float())
        loss = args.dice_weight * loss_dice + args.bce_weight * loss_bce
    ...
    def run_quick_validation(epoch):
        ...
        with torch.no_grad():
            for val_data in val_patch_loader:
                ...
                with torch.cuda.amp.autocast(enabled=args.amp):
                    logits = unwrap_output(model(val_inputs))

                val_preds = (torch.sigmoid(logits) > 0.5).float()
        ...
        print(
            f"Quick validation epoch={epoch} | pseudo_dice={metric:.6f} | "
        ...
        if metric > best_pseudo_metric:
            ...
            print(
                f"New best model saved by quick validation | pseudo_dice={metric:.6f} | epoch={epoch}",
                flush=True,
            )
    ...
    def run_full_validation(epoch, tag="Full validation"):
        ...
        with torch.no_grad():
            for val_data in val_full_loader:
                ...
                val_labels = (val_data["label"].to(device, non_blocking=True) > 0).float()

                def predictor(x):
                    return unwrap_output(model(x))
        ...
        print(
            f"{tag} epoch={epoch} | mean_dice={metric:.6f} | val_time={format_seconds(elapsed)}",
            flush=True,
        )
        ...
        if metric > best_full_metric:
            ...
            print(
                f"New best full-validation model saved | dice={metric:.6f} | epoch={epoch}",
                flush=True,
            )
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for batch_data in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                logits = unwrap_output(model(images))
                loss, loss_dice, loss_bce = compute_loss(logits, labels)
        ...
        epoch_dice /= max(step, 1)
        epoch_bce /= max(step, 1)
        ...
        print(
            ...
            f"loss={epoch_loss:.6f} | dice_loss={epoch_dice:.6f} | bce_loss={epoch_bce:.6f} | "
            f"lr={current_lr:.6g} | epoch_time={format_seconds(epoch_time)} | "
            f"elapsed={format_seconds(elapsed_total)} | ETA={format_seconds(eta_seconds)} | "
